...
export class PaymentsService {
     ...
     async create(data: CreatePaymentDto): Promise<Payment> {
          ...
          if (amount !== Number(order.totalAmount)) {
               throw new BadRequestException(`Payment amount must be equal to the total amount: ${order.totalAmount}`);
          }
     ...
     async processPayment(id: number, status: PaymentStatus): Promise<Payment> {
          ...
          return this.prisma.$transaction(async (prisma) => {
               ...
               if (status === PaymentStatus.PAID) {
                    ...
                    const payments = [...order.payments.filter(p => p.id !== id), { ...payment, status: PaymentStatus.PAID }];
